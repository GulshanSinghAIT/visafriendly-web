    ...
    <div className={styles.container}>
      ...
      <div className={styles.searchBar}>
        <div className="flex justify-between ">
          ...
          <button
            className={`${styles.uploadButton} ${resumes.length >= 5 || isUploading ? styles.disabled : ""
              }`}
             onClick={() => document.getElementById('resume-upload-input').click()}
             disabled={resumes.length >= 5 || isUploading}
            style={{
              opacity: resumes.length >= 5 || isUploading ? 0.5 : 1,
              cursor: resumes.length >= 5 || isUploading ? "not-allowed" : "pointer",
              pointerEvents: resumes.length >= 5 || isUploading ? "none" : "auto",
            }}
          >
            <input

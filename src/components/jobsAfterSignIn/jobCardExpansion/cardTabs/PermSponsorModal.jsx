...
const PermSponsorModal = ({ isOpen, onClose, companyName = "" }) => {
    ...
    // API Functions
    const fetchPermData = async (page = 1, company = '', role = '', year = '') => {
        setLoading(true);
        setError(null);
        ...
        try {
            ...
            const response = await fetch(`${process.env.REACT_APP_API_URL}/jobs/perm-reports?${params}`);
            
            if (!response.ok) {
                throw new Error('Failed to fetch PERM data');
            }
            ...
            if (data.success) {
                ...
                throw new Error(data.message || 'Failed to fetch PERM data');
            }
        } catch (err) {
            setError(err.message);
            console.error('Error fetching PERM data:', err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchAvailableYears = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/jobs/perm-reports/years`);
    ...
    useEffect(() => {
        if (isOpen) {
            fetchAvailableYears();
            fetchPermData(1, searchCompany, searchJobTitle, selectedYear);
        }
    }, [isOpen]);
    ...
    const handlePageChange = (page) => {
        fetchPermData(page, searchCompany, searchJobTitle, selectedYear);
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000] p-4">
            <div className="bg-white rounded-lg w-full max-w-7xl max-h-[90vh] overflow-hidden">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200">
                    <div>
                        <h2 className="text-xl font-semibold text-gray-900">PERM Labor Certification Report</h2>
                ...
                <div className="overflow-x-auto overflow-y-auto max-h-96">
                    {loading && (
                        <div className="flex items-center justify-center py-8">
                            ...
                            <span className="text-gray-600">Loading PERM data...</span>
                        </div>
                    )}
                        ...
                        <table className="w-full">
                            <thead className="bg-gray-50 backdrop-blur-2xl sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="text-left py-3 px-4 font-medium text-gray-700 text-sm">Decision Date</th>
                                ...
                                {permData.length === 0 ? (
                                    <tr>
                                        <td colSpan="9" className="text-center py-8 text-gray-500">
                                            No PERM data found for the selected filters.
                                        </td>
                                    </tr>
                                ) : (
                                    permData.map((item, index) => (
                                        <tr key={item.Id || index} className="border-b border-gray-100 hover:bg-gray-50">
                                            <td className="py-3 px-4 text-gray-600 text-sm">{item.year}</td>
                                            <td className="py-3 px-4 text-gray-600 text-sm">{item.Employer || 'N/A'}</td>
                                            ...
                                            <td className="py-3 px-4 text-gray-600 text-sm">{formatDate(item.decisionDate)}</td>
                                            <td className="py-3 px-4 text-sm">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                    item.caseStatus === 'Certified' || item.caseStatus === 'Approved'
                                                        ? 'bg-green-100 text-green-800' 
                                                        : item.caseStatus === 'Denied'
...
export default PermSponsorModal; 
